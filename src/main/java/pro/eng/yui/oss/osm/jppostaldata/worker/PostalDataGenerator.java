...
/**
 * 郵便番号データの JSON を生成するメインのジェネレータークラス
 */
public class PostalDataGenerator extends AbstDataGenerator {

    /**
     * 指定された都道府県（または全県）のデータを生成し、JSONファイルとして出力する
     * @param targetPrefecture 対象とする都道府県名またはコード（nullの場合は全県）
    ...
    public void generate(String targetPrefecture) throws IOException {
        ...
        // 郵便局リストの更新
        new OfficeListDataGenerator().generate(results);
        ...
        data.put("lastUpdated", Main.FORMATTER.format(ZonedDateTime.now(Main.JST)));
        data.put("status", "OK");
        ...
        simplifiedWriter.writeValue(outputPath.toFile(), data);
    ...
    private Set<PrefectureDataJsonGenerator.Result> generatePrefectureDataJson(String targetPrefecture) throws IOException {
            ...
            if (subAreaCode != null) {
                ...
                for (JsonNode subNode : subArray) {
                    ...
                    try {
                        ...
                        System.err.println(Main.FORMATTER.format(ZonedDateTime.now(Main.JST)) + " " +
    ...
    private List<PrefectureDataJsonGenerator.ResultTimestamp> loadExistPrefTimestamp(){
        List<PrefectureDataJsonGenerator.ResultTimestamp> prefectures = new ArrayList<>();

        JsonNode jsonData = loadExistingData("data/date.json");
        if (jsonData != null) {
